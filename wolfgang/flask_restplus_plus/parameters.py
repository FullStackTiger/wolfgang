...
class Parameters(Schema):
    """Parameters that uses webargs to validate REST input."""
    ...
    def __init__(self, **kwargs):
        ...
        for k in self.fields:
            if not self.fields[k].required:
                self.fields[k].allow_none = True

    def __contains__(self, field):
        """__contains__."""
        return field in self.fields

    # TODO: figure out whether we need this (seems to screw up unmarshalling)
    def make_instance(self, data):
        """
        No-op function which shadows ``ModelSchema.make_instance``.

        (when inherited classes inherit from ``ModelSchema``). Thus, we
...
class PatchJSONParameters(Parameters):
    ...
    def perform_patch(cls, operations, obj, state=None):
        ...
        Performs all necessary operations.
         ...
         by calling class methods with corresponding names.
        """
        if state is None:
            state = {}
    ...
    def replace(cls, obj, field, value, state):
        """
        Separated to provide an easy override in your Parameters.
    ...
    def test(cls, obj, field, value, state):
        """
        This is method for test operation.

        It is separated to provide a possibility to easily override it in your Parameters.
